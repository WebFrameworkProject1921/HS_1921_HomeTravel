...
function SNSCardCreateModal({ open, onClose, setCards }) {
    ...
    const [info, setInfo] = useState({
        // 지도의 초기 위치
        position: { lat: '37.577613288258206', lng: '126.97689786832184' },
        content: "경복궁"
    }
    )

    const [keyword, setKeyword] = useState('서울'); // 검색 키워드를 위한 상태값을 추가합니다.
    ...
    const handleSubmit = async () => {
        ...
        formData.append('createDate', today.getFullYear() + "." + (today.getMonth() + 1) + "." + today.getDate());
    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth={true} maxWidth={"md"} scroll={'paper'}>
            ...
            <DialogContent dividers={true}>
                ...
                <Slider dots={true} style={{ margin: "0 auto", maxWidth: 500 }} >
                    {images.map((image, key) => (
                        <Box key={key} sx={ {textAlign: "right" }}>
                            <img src={URL.createObjectURL(image)} style={{ margin: "0 auto", maxHeight: 300}}/>
                ...
                <Textarea placeholder="메모를 작성하세요" required sx={{ mb: 1 }} minRows={5} onChange={(e) => memo = e.target.value} />
...
export default SNSCardCreateModal;
